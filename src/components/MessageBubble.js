...
export function createMessageBubble(role, content = "") {
  ...
  if (isUser) {
    bubble.className = "max-w-[75%] rounded-2xl rounded-tr-sm px-4 py-2.5 bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 shadow-md text-sm leading-relaxed font-normal whitespace-pre-wrap break-words";
    bubble.textContent = content;
  } else {
    bubble.className = "max-w-[96%] rounded-2xl rounded-tl-sm px-4 py-3 bg-neutral-100 dark:bg-neutral-800/90 text-neutral-900 dark:text-neutral-100 border border-neutral-200/50 dark:border-neutral-700/50 shadow-sm text-sm markdown-body break-words";
    if (!content) {
      bubble.innerHTML = `
